...
class CarRentalAdminGUI:
    ...
    def add_car_available(self):
        """Window to add a new car as available."""
        self.add_car_window(available=True)

    def add_car_unavailable(self):
        """Window to add a new car as unavailable."""
    ...
    def add_car_window(self, available):
        ...
        def save_car():
            ...
            messagebox.showinfo("Success", f"Car {make} {model} added as {status_msg}!")
            add_car_win.destroy()

        add_car_win = tk.Toplevel(self.root)
        title = "Add Car as Available" if available else "Add Car as Unavailable"
        add_car_win.title(title)
    ...
    def view_cars(self):
        ...
        for car in self.car_data:
            tk.Label(view_win, text=f"{car['make']} {car['model']} ({car['available_from']} - {car['available_to']}) - {car['status']}").pack()
